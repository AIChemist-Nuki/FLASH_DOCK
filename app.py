...
# ---- page config + theme (must run before any other Streamlit call) ----
st.set_page_config(page_title="FLASH_DOCK", page_icon="⚡", layout="wide")
inject_theme()
...
# Page keys (internal) + nav labels and icons
PAGES = ["home", "prepare_ligand", "pocket_prediction", "docking",
         "batch_docking", "affinity_prediction", "task_manager", "author"]
PAGE_NAV_KEYS = {
    "home": "nav.home",
...
PAGE_ICONS = {
    "home": "◇", "prepare_ligand": "🧪", "pocket_prediction": "🧬",
    "docking": "🎯", "batch_docking": "📦", "affinity_prediction": "📈",
    "task_manager": "🗂", "author": "✦",
}
# Honest pipeline-stage numbers for the page-header eyebrow (None = utility page)
PAGE_STAGE = {
    "prepare_ligand": 1, "pocket_prediction": 2, "docking": 3,
    "batch_docking": 3, "affinity_prediction": 4,
}

# ---- sidebar: brand · language · navigation ----
sidebar_brand("AI Molecular Docking")
...
_nav_labels = [f"{PAGE_ICONS[p]} {t(PAGE_NAV_KEYS[p])}" for p in PAGES]
_choice = st.sidebar.radio(
    t("nav.title"), _nav_labels, index=PAGES.index(st.session_state['page'])
)
st.session_state['page'] = PAGES[_nav_labels.index(_choice)]
page = st.session_state['page']


def stage_header(page_key, title, subtitle=None):
    """Render a pipeline-stage page header for the given page."""
    n = PAGE_STAGE.get(page_key)
    eyebrow = f"{t('ui.stage')} {n:02d}" if n else t(PAGE_NAV_KEYS[page_key]).upper()
...
# ------------------------------------------------------------------------------
if page == "home":
    # --- hero: the pipeline is the thesis ---
    st.markdown(
        f'<div class="fd-hero">'
        f'<div class="fd-eyebrow">{t("home.eyebrow")}</div>'
        f'<div class="fd-hero-title">{t("home.welcome")}</div>'
        f'<p class="fd-hero-sub">{t("home.tagline")}</p>'
        f'</div>',
        unsafe_allow_html=True,
    )

    # --- pipeline overview (honest numbered sequence) ---
    _steps = [
        ("01", "🧪", "nav.prepare_ligand", "ligand.subtitle"),
        ("02", "🧬", "nav.pocket_prediction", "pocket.subtitle"),
        ("03", "🎯", "nav.docking", "dock.subtitle"),
        ("04", "📈", "nav.affinity_prediction", "affinity.subtitle"),
    ]
    _cards = "".join(
        f'<div class="fd-step"><div class="num">{t("ui.stage")} {n}</div>'
        f'<div class="ic">{ic}</div><div class="nm">{t(nm)}</div>'
        f'<div class="ds">{t(ds)}</div></div>'
        for n, ic, nm, ds in _steps
    )
    st.markdown(f'<div class="fd-section-label">{t("home.pipeline_title")}</div>',
                unsafe_allow_html=True)
    st.markdown(f'<div class="fd-pipeline">{_cards}</div>', unsafe_allow_html=True)
    ...
    _algo_cards = "".join(
        f'<div class="fd-algo"><div class="role">{role}</div>'
        f'<div class="nm"><a href="{url}" target="_blank">{name}</a></div></div>'
        for role, name, url in _algos
    )
    st.markdown(f'<div class="fd-section-label">{t("home.algorithms_title").lstrip("# ").strip()}</div>',
                unsafe_allow_html=True)
    st.markdown(f'<div class="fd-algos">{_algo_cards}</div>', unsafe_allow_html=True)

    # --- example data ---
    st.markdown('<div class="fd-section-label">EXAMPLES</div>', unsafe_allow_html=True)
...
# ------------------------------------------------------------------------------
    ...
    stage_header("prepare_ligand", t("ligand.title"), t("ligand.subtitle"))
...
# ------------------------------------------------------------------------------
    ...
    if not weights_present():
        st.error(t("pocket.error_weights_missing"))
    ...
    _display_cols = ["Protein File", "rank", "score", "volume",
                     "center_x", "center_y", "center_z"]

    def _show_best(best):
        c1, c2, c3 = st.columns(3)
        c1.metric("center_x", f"{best['center_x']:.3f}")
        c2.metric("center_y", f"{best['center_y']:.3f}")
        c3.metric("center_z", f"{best['center_z']:.3f}")

    def _best_pocket_csv(best):
        return pd.DataFrame(
            {"Pocket Name": [best["Pocket Name"]], "Center": [best["Center"]]}
        ).to_csv(index=False)

    option = st.radio(t("pocket.mode_label"), list(POCKET_OPTIONS.values()))

    # ---- single protein ----
    if option == POCKET_OPTIONS["single"]:
        pdb_file = st.file_uploader(t("pocket.uploader_pdb"), type=["pdb"])
        if pdb_file is not None and st.button(t("pocket.button_run"), key="pf_single"):
            tmp_path = None
            try:
                with tempfile.NamedTemporaryFile(suffix=".pdb", delete=False) as tmp:
                    tmp.write(pdb_file.getvalue())
                    tmp_path = tmp.name
                with st.spinner(t("pocket.spinner")):
                    df = predict_pockets([tmp_path], [pdb_file.name])
                st.success(t("pocket.success"))
                best = df.iloc[0]
                _show_best(best)
                st.write(t("pocket.preview_optimal"))
                st.dataframe(df[_display_cols], use_container_width=True, hide_index=True)
                st.download_button(
                    t("pocket.download_csv"), data=_best_pocket_csv(best),
    ...
    # ---- bundled example ----
        ...
        st.info(t("pocket.example_info"))
        if st.button(t("pocket.button_run"), key="pf_example"):
            try:
                with st.spinner(t("pocket.spinner")):
                    df = predict_pockets(
                        ["examples/pocket/protein.pdb"], ["protein_example.pdb"]
                    )
                st.success(t("pocket.success"))
                best = df.iloc[0]
                _show_best(best)
    ...
    # ---- batch ----
        ...
        uploaded_files = st.file_uploader(
            t("pocket.batch_uploader"), type=["pdb"], accept_multiple_files=True
        )
        if uploaded_files and st.button(t("pocket.batch_button")):
            paths, names = [], []
            try:
                for f in uploaded_files:
                    with tempfile.NamedTemporaryFile(suffix=".pdb", delete=False) as tmp:
                        ...
                        names.append(f.name)
                with st.spinner(t("pocket.batch_spinner", total=len(paths))):
                    result_df = predict_pockets(paths, names)
                st.success(t("pocket.batch_success"))
                st.dataframe(result_df[_display_cols], use_container_width=True, hide_index=True)
                st.download_button(
                    t("pocket.batch_download"),
    ...
    stage_header("docking", t("dock.title"), t("dock.subtitle"))
...
# ------------------------------------------------------------------------------
# Batch docking (uses existing pocket predictions)
# ------------------------------------------------------------------------------
    ...
    # 页面布局
    stage_header("batch_docking", t("batch.title"), t("batch.subtitle"))
...
# ------------------------------------------------------------------------------
    ...
    stage_header("affinity_prediction", t("affinity.title"), t("affinity.subtitle"))
...
# ------------------------------------------------------------------------------
    ...
    stage_header("task_manager", t("task.title"), t("task.subtitle"))
        ...
        if not jobs:
            ...
            for i, job in enumerate(jobs):
                ...
                with st.expander(label, expanded=False):
                    ...
                    st.markdown(status_badge(job["state"]), unsafe_allow_html=True)
